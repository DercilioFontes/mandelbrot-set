...
class Tile {
  constructor(x, y, width, height) {
    // The properties of a Tile object represent the position and size of the tile within a larger rectangle.
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
  ...
  static *tiles(width, height, numRows, numCols) {
    ...
    for (let row = 0; row < numRows; row++) {
      // height of most rows or height of last row
      let tileHeight =
        row < numRows - 1 ? rowHeight : height - rowHeight * (numRows - 1);

      for (let col = 0; col < numCols; col++) {
        // width of most columns or last column
        let tileWidth =
    ...
    this.pendingRender = Promise.all(promises)
      .then((responses) => {
        ...
        if (!this.colorTable || this.colorTable.length !== maxIterations + 1) {
          this.colorTable = new Uint32Array(maxIterations);
        }
        ...
        if (min === max) {
          if (min === this.maxIterations) {
            this.colorTable[min] = 0xff000000; // Then make them all black
          } else {
            this.colorTable[min] = 0; // Or all transparent.
          }
        } else {
